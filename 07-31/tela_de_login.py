...
def garantir_arquivo():
    if not os.path.exists("registros.txt"):
...
def login():
    ...
    try:
        ...
        for i, linha in enumerate(linhas):
            if linha.strip() == f"Usuario: {usuario}":
                if i + 1 < len(linhas) and linhas[i + 1].strip() == f"Senha: {senha}":
...
try:
    ...
    label_imagem = tk.Label(root, text="[Imagem não encontrada]", bg=COR_BEGE, fg="gray")
    label_imagem.pack(expand=True)
...
checkbox = tk.Checkbutton(
    frame_baixo,
    text="Desejo lembrar desse dispositivo",
